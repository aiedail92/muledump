...
var VAULTORDER = [19,17,16,18,20,14,12,11,13,15,9,6,4,7,10,5,2,1,3,8]
function arrangevaults(v) {
	if (VAULTORDER.length < v.length) return [0, v];
	var r = [];
...
function mail_not_verified(guid) {
  var l = $('<span>').addClass('mail-status');
  l.text('\u2718');
  l.attr('title', 'email not verified');
  return l;
}
...
function update_totals() {
	...
	for (var i in mules) {
		...
		for (var j = 0; j < m.chars.length; j++) {
			...
			if (TKGP || mules[i].opt('inv')) upd(ch.slice(4));
		}
...
Mule.prototype.query = function(ignore_cache) {
	...
	relayout();
	if (ignore_cache) this.dom.hide().empty();
	this.olddom = this.dom;
	this.dom = $('<div class="mule">');
	$('#accopts').hide().data('guid', '').appendTo($stage);
...
Mule.prototype.parse = function(data) {
	...
	if(!('VerifiedEmail' in d.Account)) {
	  this.dom.append(mail_not_verified(this.guid));
	}
	var self = this;
	$name.click(function(e) {
		...
		var $ao = $('#accopts');
		$ao.appendTo(this);
		$ao.css('left', e.pageX - self.dom.offset().left + 'px');
	...
	for (var i = 0; i < carr.length; i++) {
		...
		if (this.opt('equipment') || this.opt('inv')) {
			f = true;
		...
		if (this.opt('pcstats')) {
			f = true;
			$c.append(printstats(c, d, this.opt('goals')));
		}
		arr.push($c);
	}
	if (f) {
	...
	this.loaded = true;
	this.olddom.replaceWith(this.dom);
	relayout();
}
